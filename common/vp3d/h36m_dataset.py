...
import numpy as np
import copy
# NOTE (local modification): the camera-parameter dicts below are the only part
# of this file used by common/dataset_vp3d.py. The VP3D-internal imports and the
# Skeleton/Human36mDataset machinery are guarded so the params import cleanly
# without VideoPose3D's full package layout.
try:
    from common.skeleton import Skeleton           # noqa: F401
    from common.mocap_dataset import MocapDataset   # noqa: F401
    from common.camera import normalize_screen_coordinates, image_coordinates  # noqa: F401
    _VP3D_PKG = True
except ImportError:
    Skeleton = MocapDataset = None
    _VP3D_PKG = False
...
class Human36mDataset(MocapDataset if _VP3D_PKG else object):
